...
-- Tablas proyección: ninguna (consume datos agregados vía Kafka
...
-- ------------------------------------------------------------
-- 2. TABLAS MAESTRAS
-- Nota: este microservicio es receptor puro de eventos Kafka.
--       No mantiene proyecciones de entidades, sino agregados.
-- ------------------------------------------------------------

-- Definición de reportes del sistema
CREATE TABLE reporte (
    id              SERIAL       PRIMARY KEY,
    codigo          VARCHAR(50)  NOT NULL UNIQUE,            -- ej: OCUPACION_MENSUAL, INGRESOS_DIARIOS
...
CREATE TABLE metrica (
    ...
    periodo         DATE          NOT NULL,                  -- fecha del agregado (día, semana, mes)
    nombre_metrica  VARCHAR(80)   NOT NULL,                  -- ej: habitaciones_ocupadas, ingresos_usd
    valor           NUMERIC(15,4) NOT NULL,
    unidad          VARCHAR(30),                             -- ej: UNIDADES, USD, PORCENTAJE, MINUTOS
...
-- KPIs consolidados del hotel (indicadores de alto nivel)
CREATE TABLE kpi (
    id              SERIAL        PRIMARY KEY,
    nombre          VARCHAR(80)   NOT NULL UNIQUE,           -- ej: OCUPACION_PROMEDIO, REVPAR, ADR
    descripcion     TEXT,
...
INSERT INTO reporte (codigo, nombre, descripcion, tipo, frecuencia) VALUES
    ...
    ('BORDE_TIEMPO_REAL',   'Monitor tiempo real',          'Métricas en tiempo real (carga alta)',           'OPERACIONAL', 'TIEMPO_REAL');
...
INSERT INTO metrica (codigo_reporte, periodo, nombre_metrica, valor, unidad) VALUES
    ...
    ('OCUPACION_DIARIA',   '2024-06-01', 'duracion_media_estancia', 4.2000, 'NOCHES'),   -- caso borde: métrica extra
    ('INGRESOS_DIARIOS',   '2024-07-01', 'ingresos_total',           90.00, 'USD');       -- caso borde: día con 1 reserva
...
INSERT INTO kpi (nombre, descripcion, valor_actual, valor_objetivo, unidad, periodo) VALUES
    ...
    ('KPI_SIN_OBJETIVO',    'KPI sin meta definida (caso borde)',              42.00,  NULL, 'UNIDADES',  'ANUAL'); -- caso borde: sin objetivo
